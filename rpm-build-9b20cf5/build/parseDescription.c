...
int parseDescription(Spec spec)
{
    ...
    int previsblankline = 1;
    if ((rc = readLine(spec, STRIP_REDSPACE | STRIP_COMMENTS)) == 1) {
	nextPart = PART_NONE;
    } else {
	if (rc) {
	    return rc;
	...
	while (! (nextPart = isPart(spec->line))) {
	    ...
	    for (char *p = spec->line; *p && blankness; p++)
		if (!xisspace(*p))
		    blankness = 0;
	    if (!blankness || !previsblankline) {
		appendStringBuf(sb, spec->line);
		if (t) t->t_nlines++;
	    }
	    previsblankline = blankness;
	    if ((rc =
		 readLine(spec, STRIP_REDSPACE | STRIP_COMMENTS)) == 1) {
		nextPart = PART_NONE;
		break;
	    }
	    if (rc) {
